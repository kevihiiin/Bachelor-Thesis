...
Volcano Plot
```{r}
pdf(file=paste(output_path, '-volcano.pdf'))
png(file=paste(output_path, '-volcano.png'))
#reset par
par(mfrow=c(1,1))
...
PCA
```{r}
pdf(file=paste(output_path, '-pca.pdf', sep=''))
...
Heatmap
```{r}
pdf(file=paste(output_path, '-heatmap.pdf', sep=''))
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
ggsave(paste(output_path, '-heatmap.pdf', sep=''))
```
Advanced stuff
```{r}
